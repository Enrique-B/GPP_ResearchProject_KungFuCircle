	...
	public:
		...
		static std::vector<Elite::Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int i = 1; i < static_cast<unsigned int>(portals.size()); ++i)
			{
				...
				const Elite::Vector2 newRightLeg = portals[i].Line.p1 - apex; 
				if (Elite::Cross(rightLeg, newRightLeg) >= 0)
				{
					//2. See if new line degenerates a line segment - RIGHT
					// look if it's crossing the left leg
					if (Elite::Cross(newRightLeg, leftLeg) < 0)
					{
						// if we cross the leg
						apex += leftLeg; 
						unsigned int newIterator = leftLegIndex + 1;
						i = newIterator; 
						leftLegIndex = newIterator; 
						rightLegIndex = newIterator; 
						apexIndex = newIterator;
				...
				const Elite::Vector2 newLeftLeg = portals[i].Line.p2 - apex;
				if (Elite::Cross( leftLeg, newLeftLeg) <= 0)
				{
					...
					if (Elite::Cross(newLeftLeg, rightLeg) > 0)
					{
						// if we cross the leg
						apex += rightLeg;
						unsigned int newIterator = rightLegIndex + 1;
						i = newIterator;
						leftLegIndex = newIterator;
						rightLegIndex = newIterator;
						apexIndex = newIterator;
			...
			std::cout << "OptimizePortals vPath size: " << vPath.size() << std::endl;
			return vPath;
		}
	private:
		SSFA() {};
